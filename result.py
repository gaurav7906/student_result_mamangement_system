...
class resultClass:
    def __init__(self, root):
       ...
       title = Label(self.root, text="Add Student Result", font=("goudy old style", 20, "bold"), bg="green", fg="white").place(x=10, y=15,width=1180,height=50 )
    ...
    #=====Variables====
       ...
       #====image====
       self.bg_img = Image.open("immage/yelo.png")
        ...
        try:
           if self.var_name.get()=="":
              messagebox.showerror("error","please search student record first",parent=self.root)
               ...
               row=cur.fetchone()
               if row!=None:
                   messagebox.showerror("error","result previously exist",parent=self.root)
               else:
