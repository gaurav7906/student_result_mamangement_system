...
class StudentClass:
    ...
    #===widgets==
       ...
       self.txt_gender=ttk.Combobox(self.root,textvariable=self.var_gender,values=("choose","male","female","prefer not to say"),font=("goudy old style",15, "bold"),state='readonly',justify=CENTER)
    ...
    def delete(self):
        ...
        try:
           if self.var_rollno.get()== "":
              messagebox.showerror("error","crollno is required",parent=self.root)
           else:
           ...
           if self.var_rollno.get()== "":
              messagebox.showerror("error","roll number is required",parent=self.root)
               ...
               if row!=None:
                  ...
                  cur.execute("insert into student(rollno,name,email,gender,dob,admission,contact,course,state,city,pin,address)values(?,?,?,?,?,?,?,?,?,?,?,?)",(
                     self.var_rollno.get(),
                     self.var_Name.get(),
                     self.var_email.get(),
                     self.var_gender.get(),
           ...
           if self.var_rollno.get()== "":
              messagebox.showerror("error","roll number is required",parent=self.root)
               ...
               if row==None:
                  ...
                  cur.execute("update student set name=?,email=?,gender=?,dob=?,admission=?,contact=?,course=?,state=?,city=?,pin=?,address=? where rollno=?",(
                     self.var_rollno.get(),
                     self.var_Name.get(),
                     self.var_email.get(),
                     self.var_gender.get(),
