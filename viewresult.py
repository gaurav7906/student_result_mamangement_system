...
class reportClass:
    ...
    #===search===
       ...
       btn_clear=Button(self.root,text="Clear",font=("goudy old style",15,"bold"),bg="gray",fg="white",cursor="hand2",command=self.search).place(x=800,y=100,width=100,height=35)
              ...
              messagebox.showerror("error","search student result first",parent=self.root)
               ...
               if row==None:
                   ...
                   if op==True:
                       ...
                       con.commit()
                       messagebox.showinfo("Delete","result of student deleted successfully",parent=self.root)
                       self.clear()
        except Exception as ex:
           messagebox.showerror("error",f"error due to{str(ex)}")
